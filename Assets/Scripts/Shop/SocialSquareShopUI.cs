...
{
    /// <summary>
    /// Manages the Social Square shop UI and purchase logic.
    /// </summary>
    public class SocialSquareShopUI : MonoBehaviour
    {
        [Header("Shop Data")]
        ...
        public Sprite resourceIcon; // Assign the Heart Tokens icon in Inspector. 
        ...
        {
            int current = ResourceManager.Instance.GetResourceTotal(ResourceManager.ResourceType.HeartTokens);
            if (current >= item.price) // If the player has enough resources to buy the item, proceed with the purchase. 
            {
                ResourceManager.Instance.SpendResources(ResourceManager.ResourceType.HeartTokens, item.price); // Deduct the item's price from the player's resources. 
            ...
            else
            {
                if (rewardModal != null)
                {
                    rewardModal.Show("Not enough Heart Tokens to buy this item!", resourceIcon); // Show a warning modal if the player doesn't have enough resources. 
                }
